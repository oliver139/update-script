...
// src/index.ts
var program = new Command().name("my-update").option("-y, --yes", "Skip confirmation").version(version, "-v, --version", "Show version").helpOption("-h, --help", "Show help").showHelpAfterError();
program.parse();
...
updatingMsg();
await Promise.allSettled(Object.values(cmds).map(({ update: updateCmd }, index2) => updateCmd().then(() => {
  successResCount++;
